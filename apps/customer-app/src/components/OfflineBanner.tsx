...
// Guard against native module not being linked (old APKs / unit tests).
let NetInfo: typeof import('@react-native-community/netinfo').default | null = null;
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const mod = require('@react-native-community/netinfo');
  if (mod?.default?.addEventListener) NetInfo = mod.default;
} catch {
  // native module unavailable — banner will stay hidden
}
...
export const OfflineBanner = () => {
  ...
  useEffect(() => {
    if (!NetInfo) return; // native module not linked — skip
